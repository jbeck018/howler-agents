...
from howler_agents.persistence.repo import find_repo_root, get_db_path
...
async def _handle_howler_submit_experience(args: dict[str, Any]) -> list[types.TextContent]:
    ...
    if _remote_api_url:
        result = await _proxy_post(f"runs/{run_id}/experience", {
            "agent_id": agent_id,
...
async def _handle_howler_configure(args: dict[str, Any]) -> list[types.TextContent]:
    ...
    if _remote_api_url:
        result = await _proxy_post("config", {
...
async def _handle_howler_memory(args: dict[str, Any]) -> list[types.TextContent]:
    if _db is None:
        return [types.TextContent(type="text", text=json.dumps({"error": "Persistence not initialized. Run in local or hybrid mode."}))]
    ...
    if action == "store":
        ...
        if not key or not value:
            return [types.TextContent(type="text", text=json.dumps({"error": "key and value are required for store"}))]
        entry_id = await memory.store(
            namespace=namespace, key=key, value=value,
            tags=args.get("tags"), score=args.get("score", 0.0),
        )
        return [types.TextContent(type="text", text=json.dumps({"id": entry_id, "action": "stored", "namespace": namespace, "key": key}))]

    elif action == "retrieve":
        key = args.get("key")
        if not key:
            return [types.TextContent(type="text", text=json.dumps({"error": "key is required for retrieve"}))]
        entry = await memory.retrieve(namespace=namespace, key=key)
        if entry is None:
            return [types.TextContent(type="text", text=json.dumps({"error": f"No entry found for {namespace}/{key}"}))]
        return [types.TextContent(type="text", text=json.dumps(entry, indent=2))]

    elif action == "search":
        query = args.get("query", "")
        limit = args.get("limit", 10)
        results = await memory.search(query=query, namespace=namespace if namespace != "default" else None, limit=limit)
        return [types.TextContent(type="text", text=json.dumps({"results": results, "count": len(results)}, indent=2))]

    elif action == "list":
        limit = args.get("limit", 20)
        entries = await memory.list(namespace=namespace if namespace != "default" else None, limit=limit)
        return [types.TextContent(type="text", text=json.dumps({"entries": entries, "count": len(entries)}, indent=2))]

    elif action == "delete":
        key = args.get("key")
        if not key:
            return [types.TextContent(type="text", text=json.dumps({"error": "key is required for delete"}))]
        deleted = await memory.delete(namespace=namespace, key=key)
        return [types.TextContent(type="text", text=json.dumps({"deleted": deleted, "namespace": namespace, "key": key}))]
...
async def _handle_howler_history(args: dict[str, Any]) -> list[types.TextContent]:
    if _db is None:
        return [types.TextContent(type="text", text=json.dumps({"error": "Persistence not initialized."}))]
    ...
    if action == "runs":
        ...
        return [types.TextContent(type="text", text=json.dumps({"runs": rows, "count": len(rows)}, indent=2))]
        ...
        return [types.TextContent(type="text", text=json.dumps({"agents": rows, "count": len(rows)}, indent=2))]
        ...
        return [types.TextContent(type="text", text=json.dumps({"traces": rows, "count": len(rows)}, indent=2))]
...
async def _handle_howler_hivemind(args: dict[str, Any]) -> list[types.TextContent]:
    ...
    if action == "status":
        ...
        run_id = args.get("run_id")
        if not run_id:
            return [types.TextContent(type="text", text=json.dumps({"error": "run_id required for seed"}))]
        ...
        raw = args.get("data")
        if not raw:
            return [types.TextContent(type="text", text=json.dumps({"error": "data (JSON string) required for import"}))]
        data = json.loads(raw)
