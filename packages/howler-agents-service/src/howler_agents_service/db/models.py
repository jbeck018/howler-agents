...
class UserModel(Base):
    ...
    memberships = relationship("OrgMemberModel", back_populates="user", cascade="all, delete-orphan")
...
class AgentGroupModel(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    run_id = Column(UUID(as_uuid=True), ForeignKey("evolution_runs.id", ondelete="CASCADE"), nullable=False)
...
class AgentModel(Base):
    ...
    generation = Column(Integer, nullable=False)
    parent_id = Column(UUID(as_uuid=True), ForeignKey("agents.id", ondelete="SET NULL"), nullable=True)
    ...
    patches = relationship("FrameworkPatchModel", back_populates="agent", cascade="all, delete-orphan")
    traces = relationship("EvolutionaryTraceModel", back_populates="agent", cascade="all, delete-orphan")
